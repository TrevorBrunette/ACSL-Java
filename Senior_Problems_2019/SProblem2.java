...
public class SProblem2 {
    ...
    public static int factor(String string1, String string2){
                ...
                for(int k = 0; k < string2.length()-testString1.length()+1; k++) {
                    for(int l = k+1; l <= string2.length(); l++){
                        ...
                        if(testString1.equals(testString2)){
                            ...
                            if(testString1.length() == longestString.length() && longestString.compareTo(testString1) > 0){
                                longestString = testString1;
                            }
                        }
                    }
        ...
        String[] fourSubstrings = {string1.substring(0, string1.indexOf(longestString)), string1.substring(string1.indexOf(longestString)+longestString.length()),
                string2.substring(0, string2.indexOf(longestString)), string2.substring(string2.indexOf(longestString)+longestString.length())};
        int sum = 0;
